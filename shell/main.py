...
def build_world(
    ...
    *,
    seed: int,
) -> tuple[State, ActionSpace, MarketModel]:
    state, _ = build_state_and_discretizers(load_df, lmp_df, seed=seed)
    ...
    # NEW: seed market model
    market_model = MarketModel(action_space, market_params, seed=seed)
    return state, action_space, market_model


def train(*, n_epsiodes: int = 20, seed: int) -> tuple[TabularQLearningAgent, State, ActionSpace, MarketModel]:
    load_df, lmp_df = load_data()

    state, action_space, market_model = build_world(load_df, lmp_df, seed=seed)

    # NEW: seed agent
    agent = TabularQLearningAgent(num_actions=action_space.n_actions, seed=seed)
...
def parse_args():
    ...
    # NEW: single seed for reproducibility
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        train(n_epsiodes=args.n_episodes, seed=args.seed)
