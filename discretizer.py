...
class Discretizer:
    """
    Evaluates multiple binning methods and automatically selects the optimal one
    based on sparsity + imbalance.
    """
    ...
    # Fit edges for each method option
    ...
    # Define metrics: sparsity and imbalance
    ...
    def to_index(self, value):
        """
        Discretize single scalar value into a bin index
        """
        if self.edges_ is None:
            raise RuntimeError("Call fit() before to_index().")

        v = float(value)
        idx = np.digitize([v], self.edges_)[0] - 1
        return int(np.clip(idx, 0, len(self.edges_) - 2))


    def inverse_index(self, idx):
        """
        Return the center of the bin corresponding to index idx
        """
        if self.edges_ is None:
            raise RuntimeError("Call fit() before inverse_index().")

        idx = int(idx)
        left = self.edges_[idx]
        right = self.edges_[idx + 1]
...
if __name__ == "__main__":
    ...
    # Try discretizing a single value
    print("Index of value 30.0 =", disc.to_index(30.0))
"""